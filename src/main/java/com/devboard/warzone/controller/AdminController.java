...
public class AdminController {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(
            summary = "Get all posts by a user — ADMIN only",
            security = @SecurityRequirement(name = "bearerAuth")
    )
    public ResponseEntity<PageResponse<Post>> getUserPosts(
            @PathVariable String username,
            @RequestParam(defaultValue = "10") int limit,
            @RequestParam(defaultValue = "0")  int skip) {

        return ResponseEntity.ok(new PageResponse<>(
                postMapper.findByAuthor(username, limit, skip),
                limit,
                skip,
                postMapper.countByAuthor(username)
        ));
    }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(
            summary = "Delete all posts by a user — ADMIN only",
            security = @SecurityRequirement(name = "bearerAuth")
    )
    public ResponseEntity<Void> deleteAllUserPosts(
            @PathVariable String username) {
        postMapper.findByAuthor(username, 1000, 0)
                .forEach(post -> postMapper.delete(post.getId()));
